...
async def export_analysis(
    ...
    if format == "csv":
        out = io.StringIO()
        w = csv.writer(out)
        w.writerow(["Category", "Component", "Value", "Unit"])
        w.writerow(["Cost", "Original", float(a.cost_original or 0), a.currency])
        w.writerow(["Cost", "New Total", float(a.cost_new or 0), a.currency])
        w.writerow(["Cost", "Increase", float(a.cost_increase or 0), a.currency])
        w.writerow(["Cost", "Percentage", float(a.cost_percentage or 0), "%"])
        w.writerow(["Cost Breakdown", "Labor", float(a.cost_labor or 0), a.currency])
        w.writerow(
            [
                "Cost Breakdown",
                "Infrastructure",
                float(a.cost_infrastructure or 0),
                a.currency,
            ]
        )
        w.writerow(
        ...
        w.writerow(
            [
                "Cost Breakdown",
                "Contingency",
                float(a.cost_contingency or 0),
                a.currency,
            ]
        )
        w.writerow(
            ["Cost Breakdown", "Training", float(a.cost_training or 0), a.currency]
        )
        w.writerow(
            [
                "Cost Breakdown",
                "Documentation",
                float(a.cost_documentation or 0),
                a.currency,
            ]
        )
        w.writerow(["Time", "Original", a.time_original, "days"])
        w.writerow(["Time", "New Total", a.time_new, "days"])
        w.writerow(["Time", "Increase", a.time_increase, "days"])
        w.writerow(["Time Breakdown", "Development", a.time_development, "days"])
        w.writerow(["Time Breakdown", "Testing", a.time_testing, "days"])
        w.writerow(["Time Breakdown", "Deployment", a.time_deployment, "days"])
        w.writerow(["Time Breakdown", "Review", a.time_review, "days"])
        w.writerow(["Time Breakdown", "Documentation", a.time_documentation, "days"])
        w.writerow(["Effort", "Increase", a.effort_increase, "person-days"])
        w.writerow(["Effort Breakdown", "Senior", a.effort_senior, "person-days"])
        w.writerow(["Effort Breakdown", "Mid-level", a.effort_mid, "person-days"])
        w.writerow(["Effort Breakdown", "Junior", a.effort_junior, "person-days"])
        w.writerow(["Risk", "Level", a.risk_level, ""])
        w.writerow(["Risk", "Score", a.risk_score, "/100"])
        w.writerow(["Risk Breakdown", "Schedule", a.risk_schedule, "/100"])
        w.writerow(["Risk Breakdown", "Budget", a.risk_budget, "/100"])
        w.writerow(["Risk Breakdown", "Quality", a.risk_quality, "/100"])
        w.writerow(["Risk Breakdown", "Security", a.risk_security, "/100"])
        w.writerow(["Risk Breakdown", "Technical", a.risk_technical, "/100"])
        w.writerow(["Meta", "Change Size", a.change_size, ""])
        w.writerow(["Meta", "Confidence", a.confidence_score, "%"])
        w.writerow(["Meta", "Calculator Version", a.calculation_version, ""])
    ...
    return _load_analysis(analysis_id, db)
