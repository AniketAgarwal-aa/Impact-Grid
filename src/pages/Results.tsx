...
import {
  Download, Save, Zap, TrendingUp, Clock, Users,
...
interface AnalysisResult {
  ...
  change_type?: string;
  cost_impact: {
    original: number; new: number; increase: number; percentage: number;
...
export default function Results() {
  ...
  useEffect(() => {
    if (!id) return;
    api.getAnalysis(parseInt(id))
      .then(setAnalysis as any)
      .catch(() => toast.error("Failed to load analysis"))
      .finally(() => setLoading(false));
  }, [id]);

  if (loading)   return <PageLoader />;
  if (!analysis) return <div className="py-12 text-center text-muted-foreground">Analysis not found</div>;

  const cost       = analysis.cost_impact   || {} as any;
  const time       = analysis.time_impact   || {} as any;
  const effort     = analysis.effort_impact || {} as any;
  ...
  const burndownData = buildBurndownFromAnalysis({ time_impact: { new: time.new || 30 } });
    ...
    return (
      <div className="space-y-6 max-w-2xl mx-auto">
        <div className="flex items-start justify-between gap-4">
          ...
          <button onClick={handleExportPDF} className="flex items-center gap-2 rounded-xl border border-border px-3 py-2 text-sm hover:bg-accent print:hidden">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-2xl border border-border bg-card p-6">
            ...
            <p className="text-2xl font-bold mt-1">{format(convert(cost.increase || 0, analysis.currency))}</p>
            <p className="text-xs text-muted-foreground mt-2">+{cost.percentage || 0}% from original budget</p>
          </div>
          <div className="rounded-2xl border border-border bg-card p-6">
            ...
            <p className="text-xs text-muted-foreground mt-2">Total work required: {effort.increase || time.increase || 0} days</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            ...
            · Confidence: <span className="text-primary font-semibold">{analysis.confidence_score}%</span>
          </p>
          {fromSubmit && returnProjectId && (
            <Link to={`/projects/${returnProjectId}`} className="mt-2 inline-flex items-center gap-1 text-sm text-primary hover:underline">
              ← Back to project dashboard
            </Link>
          )}
          <div className="flex flex-wrap gap-2 mt-2">
            <StatusBadge
              status={
                analysis.change_size?.toLowerCase() === "small" ? "low"
                  : analysis.change_size?.toLowerCase() === "medium" ? "medium"
                    : analysis.change_size?.toLowerCase() === "large" ? "high"
                      : "critical"
              }
            />
            <StatusBadge status={analysis.risk_level} />
            {analysis.change_type && <StatusBadge status={analysis.change_type} />}
          </div>
        </div>
        <div className="flex gap-2 flex-wrap">
          <button onClick={handleExportPDF} className="flex items-center gap-2 rounded-xl border border-border px-3 py-2 text-sm hover:bg-accent transition-colors">
            <Download className="h-4 w-4" /> PDF
          </button>
          <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Cost Increase",      value: formatCompact(convert(cost.increase || 0, analysis.currency)), pct: `+${cost.percentage || 0}%`,   icon: TrendingUp,   color: "text-emerald-500", bg: "bg-emerald-500/10", border: "border-emerald-500/20" },
          { label: "Time Increase",      value: `+${time.increase || 0} days`,   pct: `+${time.percentage || 0}%`,   icon: Clock,         color: "text-blue-500",    bg: "bg-blue-500/10",    border: "border-blue-500/20"    },
          { label: "Total work required",value: `${effort.increase || 0} days`,  pct: `+${effort.percentage || 0}%`, icon: Users,         color: "text-purple-500",  bg: "bg-purple-500/10",  border: "border-purple-500/20"  },
          { label: "Risk Score",         value: `${analysis.risk_score || 0}/100`, pct: (analysis.risk_level || "").toUpperCase(), icon: AlertTriangle, color: "text-red-500", bg: "bg-red-500/10", border: "border-red-500/20" },
        ].map((m, i) => (
          <div key={i} className={`rounded-2xl border ${m.border} bg-card p-5 hover:shadow-lg transition-all`}>
            <div className={`flex h-10 w-10 items-center justify-center rounded-xl mb-3 ${m.bg}`}>
      ...
      <Section title="💰 Cost Analysis — 6 Component Breakdown">
        <div className="grid lg:grid-cols-3 gap-6">
          <div>
            <p className="text-sm text-muted-foreground mb-3">Before vs After comparison</p>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={costCompareData} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                <XAxis dataKey="name" fontSize={12} tick={axisTick} />
                <YAxis fontSize={11} tick={axisTick} tickFormatter={(v) => formatCompact(v)} />
                <Tooltip
          ...
          <div>
            <p className="text-sm text-muted-foreground mb-3">Cost Trend Over Time</p>
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={costTrendData} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
                ...
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                <XAxis dataKey="month" fontSize={12} tick={axisTick} />
                <YAxis fontSize={11} tick={axisTick} tickFormatter={(v) => formatCompact(v)} />
                <Tooltip contentStyle={tooltipStyle} itemStyle={tooltipItemStyle} formatter={(v: number) => [format(v), "Cost"]} />
                <Area type="monotone" dataKey="cost" name="Cost" stroke="#10b981" fillOpacity={1} fill="url(#colorCost)" strokeWidth={2.5} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          <div>
            <p className="text-sm text-muted-foreground mb-3">Cost component breakdown</p>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={costBreakdownData} layout="vertical" margin={{ top: 4, right: 16, left: 4, bottom: 4 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                <XAxis type="number" fontSize={11} tick={axisTick} tickFormatter={(v) => formatCompact(v)} />
                <YAxis type="category" dataKey="name" fontSize={11} tick={axisTick} width={90} />
                <Tooltip contentStyle={tooltipStyle} itemStyle={tooltipItemStyle} formatter={(v: number) => [format(v), ""]} cursor={{ fill: "hsl(var(--accent))", opacity: 0.5 }} />
        ...
        <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-3">
          {Object.entries(costBd).map(([k, v]) => (
            <div key={k} className="rounded-xl bg-accent/50 p-3">
              ...
              <div className="font-semibold text-sm">{format(convert(v as number, analysis.currency))}</div>
            </div>
          ))}
        </div>
      </Section>
      ...
      <Section title="⏱️ Time Analysis — 5 Phase Breakdown">
        <div className="grid lg:grid-cols-2 gap-6">
          <ResponsiveContainer width="100%" height={230}>
            <BarChart data={timeBreakdownData} layout="vertical" margin={{ top: 4, right: 16, left: 4, bottom: 4 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
              <XAxis type="number" fontSize={11} tick={axisTick} unit=" d" />
              <YAxis type="category" dataKey="name" fontSize={11} tick={axisTick} width={85} />
              <Tooltip contentStyle={tooltipStyle} itemStyle={tooltipItemStyle} formatter={(v) => [`${v} days`, ""]} cursor={{ fill: "hsl(var(--accent))", opacity: 0.5 }} />
              <Bar dataKey="value" radius={[0, 4, 4, 0]}>
          ...
          <div className="space-y-3">
            ...
            {Object.entries(timeBd).map(([k, v]) => (
              <div key={k} className="flex items-center justify-between rounded-xl bg-accent/30 px-4 py-2">
                <span className="text-sm capitalize">{k.replace(/_/g, " ")}</span>
                <span className="font-semibold text-sm">{v as number} days</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <Section title="👥 Effort Analysis — 3 Seniority Levels">
        <div className="grid lg:grid-cols-2 gap-6">
          <ResponsiveContainer width="100%" height={230}>
            <PieChart>
              <Pie
                data={effortPieData} dataKey="value" nameKey="name"
                cx="50%" cy="50%" outerRadius={90} innerRadius={40}
                label={({ name, value }) => value > 0 ? `${value}d` : ""}
                paddingAngle={3}
              >
              ...
              <Tooltip contentStyle={tooltipStyle} itemStyle={tooltipItemStyle} formatter={(v) => [`${v} days`, ""]} />
          ...
          <div className="space-y-4">
            ...
            {[
              ...
              <div key={label as string} className="rounded-xl bg-accent/30 px-4 py-3">
      ...
      <Section title="⚠️ Risk Assessment — 5 Dimensions">
        <div className="grid lg:grid-cols-3 gap-6">
          <div>
            <p className="text-sm text-muted-foreground mb-3 text-center">Risk Distribution</p>
            <ResponsiveContainer width="100%" height={260}>
              <RadarChart data={radarData}>
                <PolarGrid stroke="hsl(var(--border))" />
                <PolarAngleAxis dataKey="subject" fontSize={12} tick={axisTick} />
                <PolarRadiusAxis domain={[0, 100]} tick={false} />
                <Radar name="Risk" dataKey="A" stroke="#ef4444" fill="#ef4444" fillOpacity={0.25} strokeWidth={2} />
                <Tooltip contentStyle={tooltipStyle} itemStyle={tooltipItemStyle} />
              </RadarChart>
            </ResponsiveContainer>
          </div>
          <div>
            <p className="text-sm text-muted-foreground mb-3 text-center">Risk Trend Over Sprints</p>
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={riskTrendData} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                <XAxis dataKey="sprint" fontSize={12} tick={axisTick} />
                <YAxis fontSize={11} tick={axisTick} domain={[0, 100]} />
                <Tooltip contentStyle={tooltipStyle} itemStyle={tooltipItemStyle} />
                <Line type="monotone" dataKey="risk" name="Risk Score" stroke="#ef4444" strokeWidth={3} dot={{ r: 4, fill: "#ef4444" }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
          <div className="space-y-3">
            ...
            {[
              ...
              <ProgressBar key={label as string} label={label as string} value={val as number} showValue colorByValue />
            ))}
          </div>
        </div>
      </Section>
      ...
      <Section title="📊 Quality & Performance Impacts" defaultOpen={false}>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            <div key={l as string} className="rounded-xl bg-accent/50 p-4 text-center">
              <div className="text-2xl font-bold">{v || 0}</div>
              <div className="text-xs text-muted-foreground mt-1">{l} Impact Score</div>
              <ProgressBar value={v as number} showValue={false} className="mt-2" />
            </div>
          ))}
        </div>
      </Section>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  {["Component", "Type", "Impact", "Hours", "Priority"].map((h, i) => (
                    <th key={h} className={`py-2 text-xs font-medium text-muted-foreground uppercase ${i === 3 ? "text-right" : "text-left"} ${i === 4 ? "pl-4" : ""}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {components.map((c: ComponentImpact, i: number) => (
                  <tr key={i} className="border-b border-border/50 hover:bg-accent/30 transition-colors">
                    ...
                    <td className="py-3"><StatusBadge status={c.impact_level?.toLowerCase()} /></td>
                    <td className="py-3 text-right font-mono">{c.estimated_hours}h</td>
                    <td className="py-3 pl-4">
                      <div className="flex items-center gap-2">
                        <span className="text-xs">{c.priority || 1}</span>
                        <div className="w-16 h-1.5 bg-accent rounded-full overflow-hidden">
                          <div className={`h-full rounded-full ${c.priority === 1 ? "bg-red-500 w-full" : c.priority === 2 ? "bg-orange-500 w-2/3" : "bg-yellow-500 w-1/3"}`} />
                        </div>
                      </div>
                    </td>
                  </tr>
              ...
              <tfoot>
                <tr className="border-t border-border">
                  <td colSpan={3} className="py-3 font-medium">Total Estimated Hours</td>
                  <td className="py-3 text-right font-bold">
                    {components.reduce((sum: number, c: ComponentImpact) => sum + (c.estimated_hours || 0), 0)}h
                  </td>
                  <td />
                </tr>
              </tfoot>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {chain.split(" → ").map((m: string, i: number, arr: string[]) => (
              <div key={i} className="flex items-center gap-2">
                <span className="rounded-xl bg-primary/10 px-4 py-2 text-sm font-medium text-primary capitalize">{m}</span>
      ...
      <Section title="📉 Timeline Burndown — Planned vs Actual" defaultOpen={false}>
        <p className="text-xs text-muted-foreground mb-3">
          Tracks remaining work vs planned schedule. A line above the planned curve means you are behind schedule.
        </p>
        <ResponsiveContainer width="100%" height={250}>
          <ComposedChart data={burndownData} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
            ...
            <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
            <XAxis dataKey="day" fontSize={12} tick={axisTick} />
            <YAxis fontSize={11} tick={axisTick} unit=" d" />
            <Tooltip contentStyle={tooltipStyle} itemStyle={tooltipItemStyle} formatter={(v: number) => [`${v} days remaining`, ""]} />
            <Legend formatter={legendFormatter} />
      ...
      <Section title="🏥 Project Health Dashboard" defaultOpen={true}>
          ...
          {projectHealthData.map((item) => (
            <div key={item.name} className="rounded-xl border border-border bg-accent/30 p-4">
              ...
              <div className="w-full h-3 bg-accent rounded-full overflow-hidden">
                <div className="h-full rounded-full transition-all" style={{ width: `${Math.min(100, item.value)}%`, background: item.fill }} />
              </div>
      ...
      {costBenefitData.length > 0 && (
        <Section title="💹 Cost vs Benefit Analysis" defaultOpen={false}>
          <p className="text-xs text-muted-foreground mb-3">
            Each bubble represents an affected component. X = estimated cost, Y = estimated benefit. Larger bubbles = more hours. Top-left = best ROI.
          </p>
          <ResponsiveContainer width="100%" height={260}>
            <ScatterChart margin={{ top: 12, right: 24, bottom: 24, left: 8 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
              <XAxis
                type="number" dataKey="x" name="Cost" fontSize={11} tick={axisTick}
                tickFormatter={(v) => formatCompact(v)}
                label={{ value: "Cost (₹/hr × hrs)", position: "insideBottom", offset: -14, fontSize: 11, fill: "hsl(var(--muted-foreground))" }}
              />
              <YAxis
                type="number" dataKey="y" name="Benefit" fontSize={11} tick={axisTick}
                tickFormatter={(v) => formatCompact(v)}
              ...
              />
              <Scatter data={costBenefitData} fill="#6366f1" fillOpacity={0.8} />
            </ScatterChart>
          </ResponsiveContainer>
        </Section>
      )}
        ...
        {recs.length > 0 ? (
          <div className="space-y-2">
            {recs.map((r: string, i: number) => (
              <div key={i} className="flex items-start gap-3 rounded-xl bg-accent/50 p-3 hover:bg-accent/70 transition-colors">
      ...
      <ViewportModal open={showSave} onClose={() => setShowSave(false)} maxWidth="max-w-md" title="Save as Scenario">
        <h2 className="text-lg font-bold mb-4 pr-6">Save as Scenario</h2>
        <input
          type="text"
          value={scenarioName}
          onChange={(e) => setScenarioName(e.target.value)}
          placeholder="Scenario name..."
          className="w-full rounded-xl border border-border bg-background px-4 py-2.5 text-sm focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none mb-4 transition-all"
          onKeyDown={(e) => e.key === "Enter" && handleSave()}
        />
        <div className="flex gap-3 justify-end">
          <button onClick={() => setShowSave(false)} className="rounded-xl border border-border px-4 py-2 text-sm hover:bg-accent transition-colors">Cancel</button>
          <button onClick={handleSave} className="rounded-xl bg-primary px-6 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90 shadow-lg shadow-primary/20 transition-all">Save</button>
        </div>
      </ViewportModal>
    </div>
  );
