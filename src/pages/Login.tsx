 ...
 * Supports:
 *   - Password login
 *   - 2FA-only login (toggle: "Use Authenticator Code instead")
 *   - Inline 2FA TOTP field when 2FA is enabled on a password login
 *   - Role-tab enforcement (must match the role tab selected)
 */
import { useState } from "react";
import { Link, useLocation, useNavigate } from "react-router-dom";
import { useAuthStore } from "@/stores/authStore";
import { toast } from "@/components/common/Toast";
import { Zap, Eye, EyeOff, LogIn, Shield, ArrowLeft, KeyRound, Lock } from "lucide-react";
...
function getPreferredRole(pathname: string): string {
...
export default function Login() {
  ...
  // 2FA toggle: user can choose to log in with authenticator app code instead of password
  const [tfaMode, setTfaMode] = useState(false);
  // When password login requires 2FA, show inline TOTP field
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // In 2FA mode: send tfa_mode=true + tfa_code (no password needed)
      // In password mode + if 2FA is enabled server-side: send password + tfa_code together
      const data = await api.login(
        email,
        tfaMode ? undefined : password,
        rememberMe,
        (tfaMode || needsTfaCode) ? (tfaCode || undefined) : undefined,
        preferredRole !== "auto" ? preferredRole : undefined,
        tfaMode,
      );
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const d = data as any;
      setToken(d.access_token, d.refresh_token, d.user);
      localStorage.setItem("remember_me", rememberMe ? "true" : "false");
      toast.success(`Welcome back, ${d.user.full_name?.split(" ")[0]}! 👋`);
      navigate(getRoleRedirect(d.user.role));
    } catch (err: unknown) {
      const msg = (err as Error)?.message || "Login failed";
      // If email is not verified, show inline OTP verification UI
      if (/verify|verification|not verified/i.test(msg) || /Email not verified/i.test(msg)) {
        setVerifyMode(true);
        toast.warning("Please verify your email first. Enter the 6-digit code sent to your inbox.");
      } else if (/2FA code required/i.test(msg)) {
        // Server says 2FA is required — show inline TOTP field
        setNeedsTfaCode(true);
        toast.info("This account has 2FA enabled. Enter your authenticator code below.");
      } else {
        toast.error(msg);
      }
    } finally {
            ...
            <div className="space-y-6">
              ...
              <form onSubmit={handleVerifyEmail} className="space-y-4">
                <input
                  ...
                  placeholder="000000"
                />
                <button type="submit" className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary py-3 text-sm font-semibold text-primary-foreground hover:bg-primary/90 transition-all shadow-lg shadow-primary/20">
                  Verify
                </button>
                <button type="button" disabled={resending} onClick={handleResendVerify}
                ...
                <button type="button" onClick={() => { setVerifyMode(false); setVerifyCode(""); }}
            ...
            <div className="space-y-6">
              ...
              {forgotSent ? (
                <div className="rounded-xl bg-emerald-500/10 border border-emerald-500/20 p-5 text-center space-y-1">
                  ...
                  <p className="text-emerald-600 font-semibold text-sm">Reset code sent!</p>
          ...
          /* ── Main Login ── */
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {preferredRole !== "auto"
                    ? <>Signing in as <span className="font-semibold text-primary">{getRoleLabel(preferredRole)}</span></>
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                <Link to="/login/admin"
                  className={`rounded-lg border px-3 py-2 text-center transition-colors ${preferredRole === "admin" ? "border-primary bg-primary/10 text-primary font-semibold" : "border-border hover:bg-accent"}`}>
                  Admin
                </Link>
                <Link to="/login/pm"
                  className={`rounded-lg border px-3 py-2 text-center transition-colors ${preferredRole === "project_manager" ? "border-primary bg-primary/10 text-primary font-semibold" : "border-border hover:bg-accent"}`}>
                  Project Manager
                </Link>
                <Link to="/login/client"
                  className={`rounded-lg border px-3 py-2 text-center transition-colors ${preferredRole === "client" ? "border-primary bg-primary/10 text-primary font-semibold" : "border-border hover:bg-accent"}`}>
                  Client
                </Link>
              </div>
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                {/* Password field OR 2FA-only code field */}
                {tfaMode ? (
                  <div>
                    ...
                    <input
                      ...
                      required
                      autoFocus
                      className="w-full rounded-xl border border-border bg-background px-4 py-4 text-center text-2xl font-mono tracking-[0.5em] focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                      placeholder="000000"
                    />
                    <p className="text-xs text-muted-foreground mt-1.5">Enter the 6-digit code from your authenticator app</p>
                  </div>
                ) : (
                  <>
                    <div>
                    ...
                    {/* Inline 2FA code field when server says 2FA is required */}
                    {needsTfaCode && (
                      <div>
                        <label className="text-sm font-medium mb-1.5 block flex items-center gap-1.5">
                          <Shield className="h-3.5 w-3.5 text-primary" /> 2FA Code Required
                        </label>
                        <input
                          type="text"
                          inputMode="numeric"
                          ...
                          required
                          autoFocus
                          className="w-full rounded-xl border border-primary bg-background px-4 py-3 text-center text-2xl font-mono tracking-[0.5em] focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                          placeholder="000000"
                        />
                        <p className="text-xs text-muted-foreground mt-1.5">Enter the 6-digit code from your authenticator app</p>
                      </div>
                ...
                >
                  {submitting || isLoading
                    ...
                    : <>{tfaMode ? <KeyRound className="h-4 w-4" /> : <LogIn className="h-4 w-4" />} Sign In</>
                  }
                </button>
              </form>
              ...
              <div className="flex items-center justify-center">
                ...
                >
                  {tfaMode
                    ...
                    : <><KeyRound className="h-3 w-3" /> Use authenticator code instead</>}
                </button>
              </div>
